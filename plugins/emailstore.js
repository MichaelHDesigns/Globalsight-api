...
(function() {
  ...
  var valueKey = function(email, key) {
    return STORED_VALUE + Globaltokencore.util.twoSha256(email + SEPARATOR + key).toString('hex');
  };


  var countKey = function(email) {
    return ITEMS_COUNT + Globaltokencore.util.twoSha256(email).toString('hex');
  };
  ...
  var validatedKey = function(email) {
    return VALIDATED + Globaltokencore.util.twoSha256(email).toString('hex');
  };

  var emailToPassphrase = function(email) {
    return EMAIL_TO_PASSPHRASE + Globaltokencore.util.twoSha256(email).toString('hex');
  };
