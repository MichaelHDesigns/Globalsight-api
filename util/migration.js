...
var db = new levelup(process.argv[2] || (home + '/.Globalsight/testnet/emailstore'));
var newDb = new levelup(process.argv[3] || (home + '/.Globalsight/testnet/emailstore.migrated'));

var transformStoredValue = function(key) {
  var oldKey = key.substr('credentials-store-'.length);
  return 'emailstore-' + Globaltokencore.util.twoSha256(oldKey).toString('hex');
};
...
var transformPassphrase = function(key) {
  ...
  return 'email-to-passphrase-' + Globaltokencore.util.twoSha256(oldKey).toString('hex');
};

var transformValidated = function(key) {
  var oldKey = key.substr('validated-email-'.length);
  return 'validated-' + Globaltokencore.util.twoSha256(oldKey).toString('hex');
};
