...
PeerSync.prototype.handleBlock = function(info) {
  ...
  var blockHash = GlobaltokencoreUtil.formatHashFull(block.calcHash());
  self.log('[p2p_sync] Handle block: %s (allowReorgs: %s)', blockHash, self.allowReorgs);

  var tx_hashes = block.txs.map(function(tx) {
    return GlobaltokencoreUtil.formatHashFull(tx.hash);
  });

  self.sync.storeTipBlock({
    'hash': blockHash,
    'tx': tx_hashes,
    'previousblockhash': GlobaltokencoreUtil.formatHashFull(block.prev_hash),
