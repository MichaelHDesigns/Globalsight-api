...
// to show tx outs
var OUTS_PREFIX = 'txo-'; //txo-<txid>-<n> => [addr, glt_sat]
...
// => + glt_sat [:isConfirmed:[scriptPubKey|isSpendConfirmed:SpentTxid:SpentVout:SpentTs]
// |balance:txApperances


// TODO: use Globaltokencore networks module
...
var Globaltokencore = require('Globaltokencore'),
  ...
  util = Globaltokencore.util,
  networks = Globaltokencore.networks,
  levelup = require('levelup'),
  async = require('async'),
  config = require('../config/config'),
  assert = require('assert'),
  Script = Globaltokencore.Script,
  GlobaltokencoreUtil = Globaltokencore.util,
...
var encodedData = require('soop').load('Globaltokencore/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('Globaltokencore/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('Globaltokencore/lib/Address', {
...
TransactionDb.prototype.getStandardizedTx = function(tx, time, isCoinBase) {
  var self = this;
  tx.txid = GlobaltokencoreUtil.formatHashFull(tx.getHash());
  var ti = 0;
