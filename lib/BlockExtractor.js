...
var  Globaltokencore  = require('Globaltokencore'),
     Block    = Globaltokencore.Block,
     networks = Globaltokencore.networks,
     Parser   = Globaltokencore.BinaryParser,
     fs       = require('fs'),
     Buffer   = Globaltokencore.Buffer,
     glob     = require('glob'),
     async    = require('async');
...
BlockExtractor.prototype.readCurrentFileSync = function() {
  ...
  if(mb > 1023)
    throw new Error('CRITICAL ERROR: file size greater than 1023MB, use cat blk*.dat > bootstrap.dat to create new '
                          + 'dat files @128MB. (https://github.com/globaltoken/Globalsight-api/issues/35)');
