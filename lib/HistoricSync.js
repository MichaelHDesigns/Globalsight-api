...
var networks = Globaltokencore.networks;
...
//  var Deserialize = require('Globaltokencore/Deserialize');
...
function HistoricSync(opts) {
  ...
  this.getGlobaltokenCoreVersion(function(globaltokenVersion) {
    if (globaltokenVersion > 100000 && !config.forceRPCsync) {
      info('-------------------------------------------------------');
      info('- Globaltoken Core version >0.10 only works with RPC sync -');
      info('- Set the env variable INSIGHT_FORCE_RPC_SYNC = 1     -');
      info('-------------------------------------------------------');
      process.exit(1);
    } else {
      info('Globaltoken Core version ', globaltokenVersion);
      info('Using RPC sync ');
    }
  });
...
HistoricSync.prototype.getGlobaltokenCoreVersion = function(cb) {
  var self = this;
...
HistoricSync.prototype.getStandardizedBlock = function(b) {
  ...
  var block = {
    hash: GlobaltokencoreUtil.formatHashFull(b.getHash()),
    previousblockhash: GlobaltokencoreUtil.formatHashFull(b.prev_hash),
    time: b.timestamp,
  };
  var isCoinBase = 1;
